  ...
  switch (res.loadType) {
    ...
    case "TRACK_LOADED": {
      ...
      if (!player.playing && !player.paused && !player.queue.size) {
        player.play();
        return "> <a:music2:1027238527752216689> Adding song to queue";
      }
    ...
    case "SEARCH_RESULT": {
      ...
      collector.on("collect", async (response) => {
        ...
        if (toAdd.length === 1) {
          ...
          if (!player.playing && !player.paused && !player.queue.size) {
            await sentMsg.edit({
              content: "> <a:music2:1027238527752216689> Adding song to queue",
              embeds: [],
              components: [],
            });
            return player.play();
        ...
        embed
          .setDescription(`<a:music2:1027238527752216689> Added ${toAdd.length} songs to queue`)
          .setFooter({ text: `Requested By: ${res.tracks[0].requester.tag}` });
